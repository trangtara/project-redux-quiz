...
const CurrentAnswers = () => {
  ...
  const handleClick = answerIndex => {
    ...
    dispatch(
      quiz.actions.submitAnswer({ questionId: currentQuestionId, answerIndex })
    );
    setIsCorrect(checkIfCorrect(answerIndex));
    setIsAnswered(true);
    dispatch(
      quiz.actions.setScore({ increaseScore: checkIfCorrect(answerIndex) })
    );
  };
    ...
    <div className="answer__wrapper">
      ...
      {answers.map((answer, index) => (
        <Button
          ...
          className={
            isCorrect && index === userAnswerIndex
              ? 'answer__btn_correct'
              : !isCorrect && index === userAnswerIndex
              ? 'answer__btn_wrong'
              : 'answer__btn'
